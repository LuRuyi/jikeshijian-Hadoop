...
JOIN t_user c ON c.userid = b.userid
...
-- 评分最高的电影
create table t_top_ten_movies as
select movieid, rate from t_rating where userid = 1150 order by rate desc limit 10;

select b.movieid as movieid, c.moviename as moviename, avg(b.rate) as avgrate
from t_top_ten_movies a
